...
function Auth() {
  ...
  const handleShowConfirmPassword = () => setShowConfirmPassword(
  ...
  const handleSignIn = async (event) => {
    ...
    dispatch(
      signin({ identifier: formData.email, password: formData.password }),
    ).then((response) => {
  ...
  const handleSignUp = async (event) => {
    ...
    if (password !== confirmPassword) {
      // enqueueSnackbar("Passwords don't match. Please try again!", {
      //   variant: 'error',
      // });
      return;
    }
      ...
      dispatch(
        ...
        .then((res) => {
          ...
          if (res === 'Email is already taken.') {
            // enqueueSnackbar(
            //   'You have already registered. Please try signing in.',
            //   { variant: 'error' }
            // );
          } else {
            // enqueueSnackbar(
            //   'Registration Successfull! Please verify your email first and then login.',
            //   { variant: 'success' }
            // );
          }
        })
  ...
  const verifyEmail = () => {
    if (!user.confirmed) {
      sendEmailVerification(user.email)
        .then(() => {
          // enqueueSnackbar('Check your email for the link!', {
          //   variant: 'success',
          // });
        })
        .catch((err) => {
    ...
    <div className="auth">
      {user && !user.confirmed && (
        <div className="emailWarning">
          <b>
            To get access to all the featues of the Vendor Portal, please verify
            {' '}
            <span
              role="button"
      ...
      <div className="auth-div">
        ...
        <div className="form-div">
          ...
          <form onSubmit={handleSubmit}>
            ...
            <br />
            {!isSignup && (
              <p className="forgot-password" onClick={handleForgetPass}>
                {forgotPass}
              </p>
            )}
            {isSignup && (
            ...
            <div className="text-n-btn-div">
              <p className="signin-up-text" id={isSignup ? 'signup' : 'signin'}>
                ...
                <span className="signin-up-click" onClick={switchMode}>
                  {` ${formText.spanLink}`}
                </span>
              </p>
